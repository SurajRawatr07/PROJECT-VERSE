...
const FAQItemRow: React.FC<FAQItemProps> = memo(({ item, isOpen, onToggle, index }) => {
  return (
    <div className="card-white border border-black/8 overflow-hidden transition-all duration-200">
      <button
        type="button"
      ...
      >
        ...
        <div
          className={`w-7 h-7 rounded-full bg-[#F5F5F3] border border-black/6 flex items-center justify-center text-[#111111] shrink-0 transition-transform duration-200 ${
            isOpen ? 'rotate-180 bg-[#111111] text-white' : ''
          }`}
        >
      ...
      <AnimatePresence initial={false}>
        {isOpen && (
          ...
          >
            <div className="px-5 sm:px-6 pb-5 sm:pb-6 pt-1 text-[14px] sm:text-[15px] font-normal text-[#4A4A4A] leading-relaxed border-t border-black/5">
              {item.answer}
            </div>
          </motion.div>
        )}
    ...
    <section id="faq" className="relative w-full py-16 sm:py-20 md:py-24 px-4 sm:px-6 bg-[#F7F7F5] border-t border-black/8 font-serif">
      <div className="max-w-4xl mx-auto">
        <div className="text-center max-w-2xl mx-auto mb-10 sm:mb-14">
          ...
          <p className="text-[14px] sm:text-[15px] font-normal text-[#4A4A4A] mt-2">
